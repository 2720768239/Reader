...
- `product` **(必填)**:产品归属,如 `Claude Code` / `Claude Platform` / `API` / `Claude.ai`。无明确归属时填通用分类(如 `General`)。
...
- `sourceUrl`:原文 URL,必填。
- `category`:必填。
- `product`:**必填**。从原文判断产品归属(Claude Code / Claude Platform / API / Claude.ai 等),无明确归属填 `General`。
